...
pub fn init(token_config: &deserializer::TokensConfig) {
    ...
    for token_data_wrapper in token_data_wrapper_list.iter() {
        for template_config in token_config.templates.iter() {
            ...
            let template_content = template_content_custom(
                &mut is_data_available,
                template_config,
                token_data_wrapper,
                token_config,
                file_data_name,
                &token_data_wrapper.token_data,
            );
...
fn template_content_custom(
    ...
    let template = liquid::ParserBuilder::with_stdlib()
        .build()
        .with_context(|| format!("Liquid template build error {}", &file))
        .unwrap()
        .parse(file)
        .with_context(|| format!("Liquid template parse error {}", &file))
        .unwrap();
    ...
    Some(
        template
            ...
            .with_context(|| {
                format!(
                    "Template could not render with globals {}",
                    &globals.to_kstr()
                )
            })
            .unwrap(),
...
fn template_update_list_values(
    file_data_list: &[template::TokenData],
    current_template: &mut CustomTemplate,
    config_type: String,
    value: &String,
) {
...
fn template_list_replaced_values(
    file_data_list: &[template::TokenData],
    current_template: &mut CustomTemplate,
    config_type: String,
    templates: &[String],
...
fn template_replaced_values_single(
    template: &String,
    pure_values: &Vec<template::TokenValue>,
) -> Vec<String> {
    template_replaced_values(0, template, pure_values)
}

fn template_replaced_values(
    index: usize,
    template: &String,
    pure_values: &Vec<template::TokenValue>,
) -> Vec<String> {
    ...
    for content in pure_values {
        ...
        let current_optional =
            template_set_values(index, content, template, &template_fields, &mut globals);
...
pub fn template_set_values(
    index: usize,
    data: &template::TokenValue,
    pure_template: &String,
    fields: &Vec<deserializer::TemplateFieldData>,
    globals: &mut liquid::Object,
) -> Option<String> {
        ...
        if field_name_without_index == "variable_name" {
            ...
            match &token_value.value {
                        ...
                        if let Some(val) = value.get("value") {
                            match val {
                                serde_json::Value::Array(values) => {
                                    ...
                                    pure_value =
                                        val.get(field_index)?.get(field_name_without_index);
                                }
                ...
                deserializer::TokenDataType::None => {
                    println!(
                        "Something went wrong with template: {}, value: {}",
                        pure_template, &token_value.name
                    );
                }
            }
        }
...
pub fn template_as_values(
    template: &str,
    default_globals: &mut liquid_core::Object,
) -> Vec<deserializer::TemplateFieldData> {
    ...
    for pure in pure_values {
        ...
        if key_split.len() == 2 {
            index = key_split[1]
                .parse::<usize>()
                .with_context(|| format!("Cannot parse to usize {}", key_split[1]))
                .unwrap();
        }
