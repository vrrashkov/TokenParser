...
pub fn get_config(config_file: &str) -> deserializer::TokensConfig {
    ...
    let data = fs::read_to_string(design_tokens_config)
        .with_context(|| format!("Unable to read file from {}", config_file))
        .unwrap();

    let data_strip_comments = StripComments::new(data.as_bytes());
    let token_config: deserializer::TokensConfig = serde_yaml::from_reader(data_strip_comments)
        .with_context(|| format!("Unable to read config {}", data))
        .unwrap();
...
pub fn generate_tokens(
    tokens_config: &deserializer::TokensConfig,
) -> Vec<template::TokenDataWrapper> {
    ...
    for group in &tokens_config.global.figma_output_paths {
        ...
        if let Some(custom_file_name) = &group.combine.file_name {
            ...
            let current_file_name = Path::new(&group.combine.files.get(0).unwrap().path)
                .file_stem()
                .unwrap()
                .to_str()
                .unwrap()
        ...
        let output_path = format!(
            "{}/{}.json",
            &tokens_config.global.style_output_path, &file_name
        );
...
pub fn get_json(path: &str) -> serde_json::Value {
    let data = fs::read_to_string(path)
        .with_context(|| format!("Unable to read file from {}", path))
        .unwrap();
    let res: serde_json::Value = serde_json::from_str(&data)
        .with_context(|| format!("Unable to parse {}", data))
        .unwrap();
...
pub fn create_template(
    ...
    if let Some(template_content_value) = current_template {
        ...
        if let Some(file_name_format) = &file_name_config.format {
            deserializer::TokensConfig::format_class_name_templated(
                &mut file_name_formatted,
                file_name_format,
                type_name,
                style_name,
                &template_config.settings_general,
            );
        }
        ...
        file_name_formatted = format!(
            "{}.{}",
            file_name_formatted, &template_config.settings_general.file_name.extension
        );

        create_template_file(
            template_config,
            file_name_formatted.as_str(),
...
pub fn deserialize_token_data_value(data: &Value) -> deserializer::TokenDataType {
    //dbg!(&data);
    let value_object: deserializer::TokenDataType = serde_json::from_value(data.to_owned())
        .with_context(|| format!("Unable to read json {}", data))
        .unwrap();
...
pub fn filter_sub_properties(
    ...
    token_data_list: &mut Vec<template::TokenData>,
    path: Vec<String>,
    isFirst: bool,
) {
    ...
    if val.get("type").is_some() {
        ...
        listEnum = val
            ...
            .iter()
            .flat_map(|f| f.iter())
            .collect::<Vec<(&std::string::String, &serde_json::Value)>>();
    }
    ...
    for (ikey, ival) in listEnum {
        ...
        if (ival.is_object() && template_type.is_some()) {
            ...
            let mut token_exist = token_data_list
                .iter_mut()
                .find(|f| f.t_type == token_value.token_type.text);
