...
impl Filter for ColorFilter {
    fn evaluate(&self, input: &dyn ValueView, runtime: &dyn Runtime) -> Result<Value> {
        ...
        let formatted_color_value = utils::color_to_hex(color_value.as_str());
        ...
        if !args.format.is_empty() {
            ...
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_r_v1.to_str(),
                &rgb_r_v1,
            );
            let rgb_g_v1 = format!("{:.3}", color_rgba_v1.1);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_g_v1.to_str(),
                &rgb_g_v1,
            );
            let rgb_b_v1 = format!("{:.3}", color_rgba_v1.2);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_b_v1.to_str(),
                &rgb_b_v1,
            );
            let rgb_a_v1 = format!("{:.3}", color_rgba_v1.3);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_a_v1.to_str(),
                &rgb_a_v1,
            );
            let rgb_r_v2 = format!("{:.3}", color_rgba_v2.0);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_r_v2.to_str(),
                &rgb_r_v2,
            );
            let rgb_g_v2 = format!("{:.3}", color_rgba_v2.1);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_g_v2.to_str(),
                &rgb_g_v2,
            );
            let rgb_b_v2 = format!("{:.3}", color_rgba_v2.2);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_b_v2.to_str(),
                &rgb_b_v2,
            );
            let rgb_a_v2 = format!("{:.3}", color_rgba_v2.3);
            color_formatted = color_formatted.replace(
                deserializer::TemplateFieldVariantColor::rgb_a_v2.to_str(),
                &rgb_g_v2,
            );
            let hex = formatted_color_value.to_string();
            color_formatted = color_formatted
                .replace(deserializer::TemplateFieldVariantColor::hex.to_str(), &hex);
